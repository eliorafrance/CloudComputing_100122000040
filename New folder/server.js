...
app.post('/signup', async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      'INSERT INTO "Register" (name, email, password, confirm_password) VALUES ($1, $2, $3, $4)',
      [name, email, hashedPassword, hashedPassword] // if your table requires confirm_password column
    );
...
app.get('/contacts', async (req, res) => {
    try {
      const result = await pool.query('SELECT * FROM "ContactForm"');
  ...
  app.post('/contacts', async (req, res) => {
    ...
    try {
      const result = await pool.query(
        'INSERT INTO "ContactForm" ("name", "email", "phone", "address", "notes") VALUES ($1, $2, $3, $4, $5) RETURNING *',
  ...
  app.get('/contacts/:id', async (req, res) => {
    ...
    try {
      const result = await pool.query('SELECT * FROM "ContactForm" WHERE contact_id = $1', [id]);
  ...
  app.put('/contacts/:id', async (req, res) => {
    ...
    try {
      const result = await pool.query(
        `UPDATE "ContactForm" SET ${setClause} WHERE contact_id = $${keys.length + 1} RETURNING *`,
        [...values, id]
      );
  ...
  app.delete('/contacts/:id', async (req, res) => {
    ...
    try {
      const result = await pool.query(
        'DELETE FROM "ContactForm" WHERE "contact_id" = $1 RETURNING *',
        [id]
      );
